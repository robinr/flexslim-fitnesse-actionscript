...
package fitnesse.slim.service
{
	...
	public class SlimSocketService
	{
		private static function setupLogger(target : ILoggingTarget, isVerbose : Boolean) : void
		{
			if(isVerbose)
		...
		public static function getService(args : Array, context : ISlimServiceContext) : SocketService
		{
			const commandLine : CommandLine = CommandLine.makeCommandLine("[-v] port", args);
			
			setupLogger(context.getLoggingTarget(), commandLine.isDefined("v"));
			return new SocketService(
				int(commandLine.port),
				new SlimSocketServerFactory(),
				context.getExitFunction(),
				Log.getLogger("fitnesse")
			);
		}
		
		public static function main(args : Array) : void
		{
			SlimSocketService.getService(
				args,
				new SlimServiceContext()
			).start();
		}
	}
}